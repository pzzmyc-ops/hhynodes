...
class QwenVLDetector:
    def __init__(self):
        ...
        print(f"[QwenVL] - Mirror: https://hf-mirror.com")
        ...
        if not self.model_loaded or self.model_path != model_path:
            ...
            try:
                if os.path.exists(model_path):
                    ...
                    print(f"[QwenVL] Downloading model: {model_path}")
                    # 让HuggingFace使用默认缓存路径
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if hasattr(inputs, 'input_ids') and inputs.input_ids is not None:
                    ...
                    # 如果没有input_ids，直接使用generated_ids
                ...
                with torch.no_grad():
                    ...
                    if boxes:
                        try:
                            ...
                            # Add bboxes for this image in SAM2 format
                            ...
                            # Add empty bbox for this image due to error
                            all_bboxes.append([[]])
                    else:
                        detection_images.append(pil2tensor(pil_image))
                        empty_mask = torch.zeros((1, pil_image.size[1], pil_image.size[0]), dtype=torch.float32)
                        output_masks.append(empty_mask)
                        # Add empty bbox for this image
                        all_bboxes.append([[]])
...
class QwenVLTextGenerationNode:
    ...
    # Modified to output BBOXES format compatible with SAM2 Video Ultra
    # BBOXES format: list of [list of [bbox coordinates [x1,y1,x2,y2]]]
    INPUT_IS_LIST = True
