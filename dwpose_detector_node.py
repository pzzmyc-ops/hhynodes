...
class DWPoseDetectorNode:
    ...
    def get_model_path(self):
        # 使用ComfyUI标准模型目录路径
        comfy_models_dir = os.path.join(os.getcwd(), "models", "controlnet_aux", "hr16", "DWPose-TorchScript-BatchSize5")
        
        model_path = os.path.join(comfy_models_dir, "dw-ll_ucoco_384_bs5.torchscript.pt")
        yolo_path = os.path.join(comfy_models_dir, "yolox_l.torchscript.pt")
        
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"❌ DWPose模型文件不存在: {model_path}\n请将模型文件放置在: {comfy_models_dir}")
        
        if not os.path.exists(yolo_path):
            print(f"⚠️ YOLO模型文件不存在: {yolo_path}，将跳过人体检测步骤")
            yolo_path = None
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "DWPoseDetectorNode": "多线程 DWPose 姿态检测 by hhy"
}
